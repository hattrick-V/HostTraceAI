...
import (
	...
	"hosttrace-ai/internal/audit"
	"hosttrace-ai/internal/authctx"
	"hosttrace-ai/internal/config"
	"hosttrace-ai/internal/database"
	"hosttrace-ai/internal/einoobserve"
	"hosttrace-ai/internal/handler"
	"hosttrace-ai/internal/hitl"
	"hosttrace-ai/internal/knowledge"
	"hosttrace-ai/internal/logger"
	"hosttrace-ai/internal/mcp"
	"hosttrace-ai/internal/monitor"
	"hosttrace-ai/internal/multiagent"
	"hosttrace-ai/internal/robot"
	"hosttrace-ai/internal/security"
	"hosttrace-ai/internal/skillpackage"
	"hosttrace-ai/internal/toolguard"
...
func New(cfg *config.Config, log *logger.Logger, configPath string) (*App, error) {
	...
	registerTraceTools(mcpServer, db, log.Logger)
	...
	// 配置文件路径必须由入口传入（与 flag -config 一致）。勿再用 os.Args[1]，否则 ./hosttrace-ai --https 会把 --https 当成路径。
	...
	traceHandler := handler.NewTraceHandler(db, log.Logger)
	...
	securityToolsHandler := handler.NewSecurityToolsHandler(log.Logger)
...
func setupRoutes(
	...
	securityToolsHandler *handler.SecurityToolsHandler,
) {
	// API路由
	...
	{
		publicRoutes.GET("/security-tools", securityToolsHandler.PublicList)
		publicRoutes.GET("/security-tools/download", securityToolsHandler.PublicDownloadLatest)
		publicRoutes.GET("/security-tools/:id/download", securityToolsHandler.PublicDownload)
	}
	...
	}))
	{
		...
		protected.GET("/security-tools", securityToolsHandler.List)
		protected.POST("/security-tools", securityToolsHandler.Upload)
		protected.DELETE("/security-tools/:id", securityToolsHandler.Delete)
		...
		// 项目管理与事实黑板
		protected.GET("/trace/dashboard-summary", traceHandler.DashboardSummary)
		...
		// 溯源归档包管理。底层暂复用 chat_upload_artifacts 存储，外部 API 使用 trace-archives 语义。
		traceArchives := protected.Group("/trace-archives")
		traceArchives.GET("", chatUploadsHandler.List)
		traceArchives.GET("/export", chatUploadsHandler.Export)
		traceArchives.GET("/download", chatUploadsHandler.Download)
		traceArchives.GET("/path", chatUploadsHandler.ResolvePath)
		traceArchives.GET("/content", chatUploadsHandler.GetContent)
		traceArchives.POST("", chatUploadsHandler.Upload)
		traceArchives.POST("/mkdir", chatUploadsHandler.Mkdir)
		traceArchives.DELETE("", chatUploadsHandler.Delete)
		traceArchives.PUT("/rename", chatUploadsHandler.Rename)
		traceArchives.PUT("/content", chatUploadsHandler.PutContent)

		// 旧对话附件（chat_uploads）兼容入口；前端不再使用，后续开源版可迁移删除。
	...
	// 浏览器会隐式请求根路径图标（/favicon.ico、/apple-touch-icon.png）。
	// 显式提供这些路由，避免 404 让浏览器长期沿用旧缓存图标。
	router.StaticFile("/favicon.ico", "./web/static/favicon.ico")
	router.StaticFile("/apple-touch-icon.png", "./web/static/hosttrace-app-icon-180.png")
	router.StaticFile("/apple-touch-icon-precomposed.png", "./web/static/hosttrace-app-icon-180.png")
