...
func permissionForRequest(method, fullPath string) string {
	...
	case strings.HasPrefix(path, "/config"):
		return crudPermission(method, "config")
	case strings.HasPrefix(path, "/security-tools"):
		if method == http.MethodGet || method == http.MethodHead {
			return "config:read"
		}
		return "config:write"
	...
	case strings.HasPrefix(path, "/trace-archives"), strings.HasPrefix(path, "/chat-uploads"):
...
func isProcessGlobalMutationPath(path string) bool {
	if strings.HasPrefix(path, "/roles") || strings.HasPrefix(path, "/skills") ||
		...
		strings.HasPrefix(path, "/security-tools") || strings.HasPrefix(path, "/trace-archives") {
		return true
	}
