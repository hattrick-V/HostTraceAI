...
func TestSecurityToolsUsesConfigPermissions(t *testing.T) {
	if got := permissionForRequest(http.MethodGet, "/api/security-tools"); got != "config:read" {
		t.Fatalf("security tools read permission = %q, want config:read", got)
	}
	if got := permissionForRequest(http.MethodPost, "/api/security-tools"); got != "config:write" {
		t.Fatalf("security tools upload permission = %q, want config:write", got)
	}
	if got := permissionForRequest(http.MethodDelete, "/api/security-tools/pkg-1"); got != "config:write" {
		t.Fatalf("security tools delete permission = %q, want config:write", got)
	}
	if !isProcessGlobalMutationPath("/security-tools") {
		t.Fatal("security tools upload should be treated as process-global mutation")
	}
}

func TestTraceArchivesUsesFilesPermissions(t *testing.T) {
	if got := permissionForRequest(http.MethodGet, "/api/trace-archives"); got != "files:read" {
		t.Fatalf("GET trace archives permission=%q", got)
	}
	if got := permissionForRequest(http.MethodPost, "/api/trace-archives"); got != "files:write" {
		t.Fatalf("POST trace archives permission=%q", got)
	}
	if got := permissionForRequest(http.MethodDelete, "/api/trace-archives"); got != "files:delete" {
		t.Fatalf("DELETE trace archives permission=%q", got)
	}
	if !isProcessGlobalMutationPath("/trace-archives") {
		t.Fatal("trace archive mutations should be treated as global")
	}
}
