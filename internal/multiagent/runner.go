...
import (
	...
	"hosttrace-ai/internal/einomcp"
	"hosttrace-ai/internal/project"
	"hosttrace-ai/internal/reasoning"
...
// orchestrationOverride 非空时优先（如聊天请求体）；否则用 multi_agent.orchestration（遗留 yaml）；皆空则按 deep。
...
func RunDeepAgent(
	...
	if orchMode != "plan_execute" {
		...
		for _, sub := range effectiveSubs {
			...
			if instr == "" {
				instr = "你是 HostTraceAI 中的专业子代理，在授权渗透测试场景下协助完成用户委托的子任务。优先使用可用工具获取证据，回答简洁专业。"
			}
	...
	orchestratorName := "hosttrace-deep"
