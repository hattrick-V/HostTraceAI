...
var draftToolHints = []draftToolHint{
	...
	{Label: "威胁情报收集", Keywords: []string{"情报", "威胁情报", "threat", "ioc", "virustotal", "shodan"}, Tools: []string{"virustotal_search", "shodan_search"}},
}
...
func GenerateDraftFromLLM(ctx context.Context, req DraftRequest, oa config.OpenAIConfig, logger *zap.Logger) (*DraftResult, error) {
	...
	systemPrompt := `你是 HostTraceAI 的工作流编排助手。你必须把用户的一句话需求转换为可保存的工作流草稿 JSON。
