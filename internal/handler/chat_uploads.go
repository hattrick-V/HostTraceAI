...
var chatUploadIPv4Pattern = regexp.MustCompile(`\b(?:25[0-5]|2[0-4]\d|1?\d?\d)(?:\.(?:25[0-5]|2[0-4]\d|1?\d?\d)){3}\b`)
...
func (h *ChatUploadsHandler) Export(c *gin.Context) {
	...
	sourceFilter := strings.TrimSpace(c.Query("source"))
	search := strings.TrimSpace(c.Query("search"))
	hostIP := firstNonEmptyQuery(c, "host_ip", "target_ip", "ip", "host")
	...
	if hostIP == "" {
		hostIP = inferChatUploadsHostIP(files)
	}
	filename := chatUploadsExportFilename(files, projectFilter, conversationFilter, hostIP)
...
func chatUploadsExportFilename(files []ChatUploadFileItem, projectFilter, conversationFilter, hostIP string) string {
	if normalized := sanitizeChatUploadsHostIP(hostIP); normalized != "" {
		return time.Now().Format("20060102") + "+" + normalized + ".zip"
	}
	nameParts := []string{"chat-files"}
	if strings.TrimSpace(projectFilter) != "" {
		nameParts = append(nameParts, "project-"+strings.TrimSpace(projectFilter))
	}
	if strings.TrimSpace(conversationFilter) != "" {
		nameParts = append(nameParts, "conversation-"+strings.TrimSpace(conversationFilter))
	}
	nameParts = append(nameParts, time.Now().Format("20060102-150405"))
	if len(files) > 0 {
		if ip := inferChatUploadsHostIP(files); ip != "" {
			return time.Now().Format("20060102") + "+" + ip + ".zip"
		}
	}
	return strings.Join(nameParts, "-") + ".zip"
}

func inferChatUploadsHostIP(files []ChatUploadFileItem) string {
	found := make(map[string]bool)
	for _, item := range files {
		fields := []string{item.RelativePath, item.Name, item.ConversationID, item.ProjectID, item.SubPath}
		for _, field := range fields {
			matches := chatUploadIPv4Pattern.FindAllString(field, -1)
			for _, match := range matches {
				if ip := sanitizeChatUploadsHostIP(match); ip != "" {
					found[ip] = true
				}
			}
		}
...
func sanitizeChatUploadsHostIP(hostIP string) string {
	hostIP = strings.TrimSpace(hostIP)
	if hostIP == "" {
		return ""
	}
	if match := chatUploadIPv4Pattern.FindString(hostIP); match != "" {
		return match
	}
	replacer := strings.NewReplacer("/", "_", "\\", "_", ":", "_", "*", "_", "?", "_", "\"", "_", "<", "_", ">", "_", "|", "_", " ", "_")
	hostIP = strings.Trim(replacer.Replace(hostIP), "._-+")
