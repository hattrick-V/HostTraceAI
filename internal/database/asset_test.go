...
func TestFofaAssetIgnoresInvalidOptionalStructuredFields(t *testing.T) {
	db, err := NewDB(filepath.Join(t.TempDir(), "imported-asset-validation.db"), zap.NewNop())
	if err != nil {
		t.Fatal(err)
	}
	defer db.Close()
	...
	result, err := db.UpsertAssets([]*Asset{asset}, "")
	if err != nil || result.Created != 1 {
		t.Fatalf("Imported asset with dirty optional domain was not created: result=%#v err=%v", result, err)
	}
	if asset.Domain != "" || asset.IP != "203.0.113.59" {
		t.Fatalf("Imported structured fields were not sanitized: %#v", asset)
	}
}
...
func TestAssetUpsertDeduplicatesAndUpdates(t *testing.T) {
	...
	first := &Asset{Host: "https://example.com", Domain: "Example.COM", Port: 443, Protocol: "HTTPS", Title: "Old", Source: "imported"}
	...
	second := &Asset{Domain: "example.com", Port: 443, Protocol: "https", Title: "New", Server: "nginx", Source: "imported"}
...
func TestAssetListFlexibleFiltersAndOldestScanPagination(t *testing.T) {
	...
	assets := []*Asset{
		{IP: "192.0.2.1", Port: 443, Protocol: "https", Source: "imported", Tags: []string{"prod"}},
	...
	port := 443
	filtered, total, err := db.ListAssets(20, 0, AssetListFilter{Source: "imported", Port: &port, LastScanBefore: &recent}, access)
