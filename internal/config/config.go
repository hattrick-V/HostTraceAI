...
type MultiAgentConfig struct {
	...
	// Orchestration 已弃用：保留仅兼容旧版 config.yaml；编排由聊天请求体 orchestration 决定，未传时按 deep。
...
// RobotsConfig 机器人配置（企业微信、钉钉、飞书、Telegram、Slack、Discord、QQ 等）
...
func (c RobotsConfig) ServiceAccountUserIDs() map[string]string {
	...
	for _, platform := range []string{"wecom", "dingtalk", "lark", "telegram", "slack", "discord", "qq"} {
...
const hitlAuditAgentPromptBase = `你是 HostTraceAI 人机协同审计 Agent。审查 Agent 即将执行的工具调用是否会对系统造成实质性损害。
...
func PrintMCPConfigJSON(mcp MCPConfig) {
	...
	out := map[string]interface{}{
		"mcpServers": map[string]interface{}{
			"hosttrace-ai": serverEntry,
		},
	}
	b, _ := json.MarshalIndent(out, "", "  ")
	fmt.Println("[HostTraceAI] MCP 配置（可复制到 Cursor / Claude Code 使用）：")
