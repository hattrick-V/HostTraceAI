...
Chrome / Edge（Chromium）DevTools 扩展：在开发者工具中捕获 **Network** 流量，发送到 HostTraceAI 进行 AI 辅助分析，并按生产场景做了性能与体验优化。
...
2. 选择目录：`plugins/browser-extension/hosttrace-browser-extension/`
3. 打开目标页面 → **F12** → 顶部 Tab **HostTraceAI**
...
- 权限申请直接绑定 Validate 点击事件，仅申请当前 HostTraceAI 服务 origin；已授权地址不会重复弹窗
...
升级并重启 HostTraceAI 服务。新版服务会自动识别格式合法的 Chrome/Edge 扩展 Origin，无需复制插件 ID 或配置 CORS 白名单；插件首次 Validate 时仍会请求访问目标服务地址的浏览器权限。

**Validate 要求允许访问 HostTraceAI 服务？**  
在浏览器弹出的权限框中允许访问当前服务地址。插件只按需申请所填写的服务 origin，不需要开启全站访问。如果未出现权限框，请在 `chrome://extensions/` 重新加载扩展，完全关闭 DevTools 后再打开并点击 Validate。

**HTTPS 显示无法连接？**  
浏览器扩展不能绕过 Chromium 的 TLS 校验。请先在浏览器中打开服务地址并信任证书；生产环境建议使用包含服务 IP/域名 SAN 的受信任证书。
...
```bash
bash plugins/browser-extension/hosttrace-browser-extension/package.sh
# → dist/hosttrace-browser-extension.zip
```

图标从项目根 `web/static/hosttrace-logo-light-512.png` 生成：

```bash
python - <<'PY'
from PIL import Image
logo = Image.open('web/static/hosttrace-logo-light-512.png').convert('RGBA')
icons = 'plugins/browser-extension/hosttrace-browser-extension/icons'
for size in (16, 48, 128):
    logo.resize((size, size), Image.LANCZOS).save(f'{icons}/icon{size}.png')
PY
```
...
| 能力 | 说明 |
|------|------|
| 流量来源 | DevTools Network（XHR / Fetch） |
| 连接配置 | Panel 内（Validate 成功后可折叠） |
| HTTP 格式 | 展示与 Prompt 均归一化为 HTTP/1.1 |
| 项目 / 角色 / 模式 | Send 弹窗选择 |
| SSE 输出 | Progress + Final 分区 |
| 捕获开关 | ● 捕获中 / ○ 已暂停 |
| 数据上限 | 200 条捕获/Tab、50 条测试记录、进度 512KB（全部内存态） |
