...
const CHAT_TASK_SYNC_CHANNEL_NAME = 'hosttrace-chat-task-sync-v1';
...
function addProgressMessage() {
    ...
    const progressTitleText = typeof window.t === 'function' ? window.t('chat.progressInProgress') : '溯源调查进行中…';
...
function startProcessDetailsLatestFollow(assistantMessageId, options) {
    ...
    const followLatest = function () {
        ...
        if (window.HostTraceChatScroll &&
            typeof window.HostTraceChatScroll.scrollIfPinned === 'function') {
            window.HostTraceChatScroll.scrollIfPinned(true);
        }
    };
...
function toggleProcessDetails(progressId, assistantMessageId) {
    ...
    if (timeline && timeline.classList.contains('expanded')) {
        setTimeout(() => {
            if (window.HostTraceChatScroll && typeof window.HostTraceChatScroll.scrollIntoViewIfFollowing === 'function') {
                window.HostTraceChatScroll.scrollIntoViewIfFollowing(detailsContainer, { behavior: 'smooth', block: 'nearest' });
...
function handleStreamEvent(event, progressElement, progressId, 
    ...
    switch (event.type) {
        ...
        case 'done':
            ...
            if (doneTitle) {
                doneTitle.textContent = '✅ ' + (typeof window.t === 'function' ? window.t('chat.penetrationTestComplete') : '溯源调查完成');
            }
            // 更新对话ID
            if (event.data && event.data.conversationId) {
                setCurrentConversationIdFromStream(event.data.conversationId);
...
function describeHitlApprovalRequest(data) {
    ...
    let question = hitlApprovalTemplate('hitl.requestGeneric', '允许 HostTraceAI 调用 {{tool}}？', { tool: rawToolName });
    let primary = '';
    let kind = 'generic';
    if (isBrowser) {
        kind = 'browser';
        question = url
            ? (url.length > 160
                ? hitlApprovalTranslate('hitl.requestVisitLongUrl', '允许 HostTraceAI 访问此地址？')
                : hitlApprovalTemplate('hitl.requestVisitUrl', '允许 HostTraceAI 访问 {{url}}？', { url: url }))
            : hitlApprovalTranslate('hitl.requestBrowser', '允许 HostTraceAI 使用浏览器？');
        primary = url;
    } else if (isCommand) {
        kind = 'command';
        question = hitlApprovalTranslate('hitl.requestCommand', '允许 HostTraceAI 执行这条命令？');
        primary = command;
    } else if (isFile) {
        kind = 'file';
        question = path
            ? (path.length > 160
                ? hitlApprovalTranslate('hitl.requestModifyLongPath', '允许 HostTraceAI 修改此文件？')
                : hitlApprovalTemplate('hitl.requestFile', '允许 HostTraceAI 修改 {{path}}？', { path: path }))
            : hitlApprovalTranslate('hitl.requestFiles', '允许 HostTraceAI 修改文件？');
        primary = path;
    }
    return {
...
async function reconcileConversationAfterTaskReplay(conversationId, keepFollowing) {
    ...
    if (keepFollowing && window.HostTraceChatScroll) {
        if (typeof window.HostTraceChatScroll.settleToBottomIfFollowing === 'function') {
            window.HostTraceChatScroll.settleToBottomIfFollowing(24);
        } else if (typeof window.HostTraceChatScroll.forceScrollToBottom === 'function') {
            window.HostTraceChatScroll.forceScrollToBottom(false);
        }
    }
...
async function attachRunningTaskEventStream(conversationId) {
    ...
    const attachPromise = (async function () {
        try {
            ...
            if (window.HostTraceChatScroll && typeof window.HostTraceChatScroll.onTaskEventStreamBegin === 'function') {
                window.HostTraceChatScroll.onTaskEventStreamBegin(conversationId, asEl.id, progressId);
            }
            ...
            if (
                window.HostTraceChatScroll &&
                typeof window.HostTraceChatScroll.settleToBottomIfFollowing === 'function' &&
                (typeof window.captureScrollPinState !== 'function' || window.captureScrollPinState())
            ) {
                window.HostTraceChatScroll.settleToBottomIfFollowing(12);
            } else if (
                window.HostTraceChatScroll &&
                typeof window.HostTraceChatScroll.forceScrollToBottom === 'function' &&
                (typeof window.captureScrollPinState !== 'function' || window.captureScrollPinState())
            ) {
                window.HostTraceChatScroll.forceScrollToBottom(false);
            }
            ...
            if (!response.ok) {
                ...
                if (window.HostTraceChatScroll && typeof window.HostTraceChatScroll.onTaskEventStreamEnd === 'function') {
                    window.HostTraceChatScroll.onTaskEventStreamEnd();
                }
                await reconcileConversationAfterTaskReplay(conversationId, true);
                return false;
            }
            ...
            if (replaySawDone && typeof window.loadConversation === 'function' && window.currentConversationId === conversationId) {
                ...
                if (
                    ...
                    window.HostTraceChatScroll.settleToBottomIfFollowing(18);
                } else if (
                    keepFollowingFinalRender &&
                    window.HostTraceChatScroll &&
                    typeof window.HostTraceChatScroll.forceScrollToBottom === 'function'
            ...
            if (ownsCurrentAttach && window.HostTraceChatScroll && typeof window.HostTraceChatScroll.onTaskEventStreamEnd === 'function') {
                window.HostTraceChatScroll.onTaskEventStreamEnd();
            }
...
function isToolGuardBlockedResult(value, depth, allowLegacy) {
    ...
    if (value._meta && value._meta['hosttrace.ai/blocked'] === true) return true;
...
if (typeof document !== 'undefined' && !document.__hostTraceToolCallDetailToggleBound) {
    document.__hostTraceToolCallDetailToggleBound = true;
