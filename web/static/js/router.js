...
const HOSTTRACE_RETIRED_PAGES = {
    'knowledge-management': 'mcp-management',
    'knowledge-retrieval-logs': 'mcp-monitor',
    'workflows': 'tasks'
};

function normalizeHostTracePage(pageId) {
    return HOSTTRACE_RETIRED_PAGES[pageId] || pageId;
}
...
function initRouter() {
    ...
    if (hash) {
        const hashParts = hash.split('?');
        let pageId = normalizeHostTracePage(hashParts[0]);
        if (pageId && ['dashboard', 'chat', 'hitl', 'tool-guard', 'asset-overview', 'asset-library', 'vulnerabilities', 'chat-files', 'mcp-monitor', 'mcp-management', 'knowledge-management', 'knowledge-retrieval-logs', 'roles-management', 'platform-rbac', 'workflows', 'skills-monitor', 'skills-management', 'agents-management', 'settings', 'tasks'].includes(pageId)) {
            switchPage(pageId);
            if (pageId === 'chat') {
...
function switchPage(pageId) {
    pageId = normalizeHostTracePage(pageId);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('hashchange', function() {
        ...
        if (pageId && ['dashboard', 'chat', 'hitl', 'tool-guard', 'asset-overview', 'asset-library', 'tasks', 'workflows', 'vulnerabilities', 'chat-files', 'mcp-monitor', 'mcp-management', 'knowledge-management', 'knowledge-retrieval-logs', 'roles-management', 'platform-rbac', 'skills-monitor', 'skills-management', 'agents-management', 'settings'].includes(pageId)) {
            switchPage(pageId);
            if (pageId === 'chat') {
                scheduleChatConversationFromHash(0);
            }
