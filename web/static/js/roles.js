...
async function loadRoles() {
    ...
    if (rolesList) {
        rolesList.innerHTML = `<div class="roles-state-panel roles-state-loading"><span class="roles-state-spinner" aria-hidden="true"></span><span>${escapeHtml(_t('roles.loading'))}</span></div>`;
    }
    if (rolesSummary) {
        rolesSummary.innerHTML = `<div class="roles-summary-loading">${escapeHtml(_t('roles.loading'))}</div>`;
    }
    try {
        ...
        if (rolesSummary) {
            rolesSummary.innerHTML = `<div class="roles-summary-error">${escapeHtml(loadFailedLabel)}</div>`;
        }
        if (rolesList) {
            rolesList.innerHTML = `
                <div class="roles-state-panel roles-state-error" role="alert">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 9v4m0 4h.01M10.3 3.7 2.6 17a2 2 0 0 0 1.7 3h15.4a2 2 0 0 0 1.7-3L13.7 3.7a2 2 0 0 0-3.4 0Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    <strong>${escapeHtml(_t('roles.loadErrorTitle'))}</strong>
                    <span>${escapeHtml(_t('roles.loadErrorHint'))}</span>
                    <button type="button" class="btn-secondary" onclick="loadRoles()">${escapeHtml(_t('roles.retry'))}</button>
                </div>`;
        }
        return null;
    }
...
async function refreshRoles() {
    ...
    if (currentPage === 'roles-management') {
        if (loadedRoles) renderRolesList();
    }
    // 始终更新侧边栏角色选择列表
    renderRoleSelectionSidebar();
    if (loadedRoles) showNotification(_t('roles.refreshed'), 'success');
}
...
function renderRolesSummary() {
    ...
    const workflows = roles.filter(role => !!role.workflow_id).length;
    const items = [
        ['roles.totalCount', total],
        ['roles.enabledCount', enabled],
        ['roles.disabledCount', disabled],
        ['roles.workflowCount', workflows]
    ];
    summary.innerHTML = items.map(([label, value]) => `
        <div class="roles-summary-item">
            <span>${escapeHtml(_t(label))}</span>
            <strong>${value}</strong>
        </div>`).join('');
}
...
function renderRolesList() {
    ...
    const resultCount = document.getElementById('roles-result-count');
    if (resultCount) {
        resultCount.textContent = _t('roles.resultCount', { count: filteredRoles.length });
    }

    if (filteredRoles.length === 0) {
        rolesList.innerHTML = `<div class="roles-state-panel roles-state-empty">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="10" cy="8" r="3" stroke="currentColor" stroke-width="2"/><path d="M4 19c.7-3.3 2.7-5 6-5s5.3 1.7 6 5M17 8h4M19 6v4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
            <strong>${escapeHtml(rolesSearchKeyword ? _t('roles.noMatchingRoles') : _t('roles.noRoles'))}</strong>
            <span>${escapeHtml(rolesSearchKeyword ? _t('roles.noMatchingRolesHint') : _t('roles.noRolesHint'))}</span>
        </div>`;
        return;
    }
    ...
    rolesList.innerHTML = sortedRoles.map(role => {
        ...
        return `
        <article class="role-card" data-status="${role.enabled !== false ? 'enabled' : 'disabled'}">
            ...
            <dl class="role-card-facts">
                <div>
                    <dt>${escapeHtml(_t('roleModal.toolsLabel'))}</dt>
                    <dd>${toolsDisplay}</dd>
                </div>
                <div>
                    <dt>${escapeHtml(_t('roles.workflowLabel'))}</dt>
                    <dd>${escapeHtml(role.workflow_id || _t('roles.noWorkflow'))}</dd>
                </div>
                <div>
                    <dt>${escapeHtml(_t('roles.promptLabel'))}</dt>
                    <dd>${escapeHtml(role.user_prompt ? _t('roles.configured') : _t('roles.notConfigured'))}</dd>
                </div>
            </dl>
            <div class="role-card-readiness" aria-label="${escapeAttr(_t('roles.configurationStatus'))}">
                <span class="${plainDesc ? 'is-ready' : ''}"></span>
                <span class="${role.user_prompt ? 'is-ready' : ''}"></span>
                <span class="${role.workflow_id || role.name === '默认' ? 'is-ready' : ''}"></span>
            </div>
            <div class="role-card-actions">
                <button type="button" class="btn-secondary btn-small" onclick="editRole(${escapeJsStringAttr(role.name)})">${_t('common.edit')}</button>
                ${role.name !== '默认' ? `<button type="button" class="btn-secondary btn-small btn-danger" onclick="deleteRole(${escapeJsStringAttr(role.name)})">${_t('common.delete')}</button>` : ''}
            </div>
        </article>
    `;
    }).join('');
...
function updateRoleEditorPreview() {
    const nameInput = document.getElementById('role-name');
    const descriptionInput = document.getElementById('role-description');
    const iconInput = document.getElementById('role-icon');
    const enabledInput = document.getElementById('role-enabled');
    const previewName = document.getElementById('role-preview-name');
    const previewDescription = document.getElementById('role-preview-description');
    const previewIcon = document.getElementById('role-preview-icon');
    const previewStatus = document.getElementById('role-preview-status');
    ...
    previewName.textContent = name || _t('roleModal.previewUnnamed');
    previewDescription.textContent = description || _t('roleModal.previewDescription');
    if (previewIcon) previewIcon.textContent = icon ? Array.from(icon)[0] : (name ? Array.from(name)[0].toUpperCase() : 'R');
    if (previewStatus) {
        previewStatus.textContent = enabled ? _t('roles.enabled') : _t('roles.disabled');
        previewStatus.classList.toggle('is-enabled', enabled);
        previewStatus.classList.toggle('is-disabled', !enabled);
    }
}

function updateRolePromptCount() {
    const prompt = document.getElementById('role-user-prompt');
    const count = document.getElementById('role-prompt-count');
    if (prompt && count) count.textContent = _t('roleModal.characterCount', { count: prompt.value.length });
}

function syncRoleWorkflowPolicyState() {
    const workflow = document.getElementById('role-workflow-id');
    const policy = document.getElementById('role-workflow-policy');
    if (!workflow || !policy) return;
    policy.disabled = !workflow.value;
    syncRoleModalSelect('role-workflow-policy');
}

function setRoleSaveState(saving) {
    roleSaveInFlight = saving;
    const button = document.getElementById('role-save-btn');
    if (!button) return;
    button.disabled = saving;
    button.classList.toggle('is-saving', saving);
    button.textContent = saving ? _t('roleModal.saving') : _t('common.save');
}

function openRoleEditorModal() {
    const body = document.querySelector('#role-modal .role-editor-body');
    const closeButton = document.querySelector('#role-modal .role-editor-close');
    if (body) body.scrollTop = 0;
    openAppModal('role-modal', { focus: false });
...
async function showAddRoleModal() {
    ...
    syncRoleWorkflowPolicyState();
    updateRoleEditorPreview();
    updateRolePromptCount();
    roleModalDirty = false;
    setRoleSaveState(false);
    openRoleEditorModal();
}
...
// 关闭角色模态框
function closeRoleModal(force) {
    if (!force && roleModalDirty && !window.confirm(_t('roleModal.discardChanges'))) return;
    roleModalDirty = false;
    closeAllRoleModalSelects();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (roleModal) {
        const editableIds = new Set([
            'role-name', 'role-description', 'role-icon', 'role-user-prompt',
            'role-enabled', 'role-workflow-id', 'role-workflow-policy'
        ]);
        const handleRoleEditorChange = (event) => {
            if (!editableIds.has(event.target.id)) return;
            roleModalDirty = true;
            updateRoleEditorPreview();
            if (event.target.id === 'role-user-prompt') updateRolePromptCount();
            if (event.target.id === 'role-workflow-id') syncRoleWorkflowPolicyState();
        };
        roleModal.addEventListener('input', handleRoleEditorChange);
        roleModal.addEventListener('change', handleRoleEditorChange);
    }
});
