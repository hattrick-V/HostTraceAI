// 溯源归档包（trace_archives）管理

const TRACE_ARCHIVES_API = '/api/trace-archives';

let chatFilesCache = [];
/** 后端 GET /api/trace-archives 返回的目录相对路径（含空文件夹），与 files 合并成树 */
...
/** 非空时，下一次上传归档包落到此相对路径（trace_archives 下目录），如 2026-03-21/uuid/sub */
let chatFilesPendingUploadDir = '';
/** 归档页面向服务器上传进行中，避免重复选择并禁用顶栏按钮 */
...
function initChatFilesPage() {
    ...
    if (sel) {
        try {
            ...
            if (v === 'target' || v === 'none' || v === 'date' || v === 'conversation' || v === 'folder') {
                sel.value = v;
            } else {
                sel.value = 'target';
            }
...
async function loadChatFilesPage() {
    ...
    params.set('pageSize', (groupMode === 'folder' || groupMode === 'target') ? 'all' : String(chatFilesPageSize));
    let url = TRACE_ARCHIVES_API;
...
async function exportChatFiles() {
    ...
    const url = TRACE_ARCHIVES_API + '/export' + (params.toString() ? ('?' + params.toString()) : '');
    try {
        const res = await apiFetch(url);
        if (!res.ok) {
...
function chatFilesTotalPages() {
    const mode = chatFilesGetGroupByMode();
    if (mode === 'folder' || mode === 'target') return 1;
...
function renderChatFilesPagination() {
    ...
    if (!pager) return;
    const groupMode = chatFilesGetGroupByMode();
    if (groupMode === 'folder' || groupMode === 'target' || chatFilesTotal <= 0) {
        pager.hidden = true;
        pager.innerHTML = '';
        return;
    }
...
async function copyChatFilePathIdx(idx) {
    ...
    const text = (f.absolutePath && String(f.absolutePath).trim())
        ...
        : ('trace_archives/' + String(f.relativePath || '').replace(/^\/+/, ''));
...
function chatFilesGetGroupByMode() {
    ...
    if (v === 'target' || v === 'date' || v === 'conversation' || v === 'folder' || v === 'none') return v;
    return 'target';
}
...
function chatFilesSourceLabel(source) {
    ...
    return (typeof window.t === 'function') ? window.t('chatFilesPage.sourceUpload') : '人工归档';
}
...
function chatFilesTreeDisplayName(name) {
    if (name === CHAT_FILES_TREE_UPLOAD_ROOT) {
        return (typeof window.t === 'function') ? window.t('chatFilesPage.treeUploadsRoot') : '归档包';
    }
...
function chatFilesBuildGroups(files, mode) {
    ...
    files.forEach(function (f, idx) {
        ...
        if (mode === 'date') {
            ...
            key = chatFilesTargetKeyForFile(f);
        } else {
...
function chatFilesTargetKeyForFile(f) {
    const explicit = String((f && (f.hostIp || f.hostIP || f.targetIp || f.target || f.host)) || '').trim();
    if (explicit) return explicit;
    const name = String((f && f.name) || '').trim();
    const rel = String((f && f.relativePath) || '').replace(/\\/g, '/');
    const plusMatch = name.match(/^\d{8}\+(.+?)\.(zip|7z|rar|tar|tgz|gz)$/i);
    if (plusMatch && plusMatch[1]) return plusMatch[1].trim();
    const scanText = [name, rel, String((f && f.subPath) || '')].join(' ');
    const ipMatch = scanText.match(/\b(?:(?:25[0-5]|2[0-4]\d|1?\d?\d)\.){3}(?:25[0-5]|2[0-4]\d|1?\d?\d)\b/);
    if (ipMatch) return ipMatch[0];
    const hostMatch = name.match(/^\d{8}\+([A-Za-z0-9][A-Za-z0-9._-]{1,253})\./);
    if (hostMatch && hostMatch[1]) return hostMatch[1];
    const conv = f && f.conversationId;
    if (conv && conv !== '_manual' && conv !== '_new') return 'conversation:' + conv;
    return 'unknown';
}

function chatFilesTargetHeading(key, items) {
    const unknown = (typeof window.t === 'function') ? window.t('chatFilesPage.targetUnknown') : '未识别目标';
    if (!key || key === 'unknown' || key === '—') return { text: unknown, title: '' };
    if (String(key).indexOf('conversation:') === 0) {
        const convID = String(key).slice('conversation:'.length);
        const conv = chatFilesConversationDisplay(convID);
        const prefix = (typeof window.t === 'function') ? window.t('chatFilesPage.targetFromConversation') : '会话';
        return { text: prefix + ' · ' + conv.text, title: conv.title || convID };
    }
    return { text: String(key), title: String(key) };
}

function chatFilesTargetGroupStats(group) {
    ...
    items.forEach(function (item) {
        ...
        latest = Math.max(latest, Number(f.modifiedUnix || 0));
        if (f.conversationId) convs.add(String(f.conversationId));
        if (f.date) dates.add(String(f.date));
        if (f.source) sources.add(String(f.source));
    });
    return {
        size: size,
        latest: latest,
...
function renderChatFilesTable() {
    ...
    // 「按文件夹」模式下即使尚无文件，也要显示归档路径栏与「新建文件夹」，否则无法先建目录
    ...
    } else if (groupMode === 'target') {
        const groups = chatFilesBuildGroups(chatFilesDisplayed, groupMode);
        const tPackage = (typeof window.t === 'function') ? window.t('chatFilesPage.targetPackageCount') : '归档包';
        const tSize = (typeof window.t === 'function') ? window.t('chatFilesPage.targetTotalSize') : '总大小';
        const tLatest = (typeof window.t === 'function') ? window.t('chatFilesPage.targetLatest') : '最近更新';
        const tConvs = (typeof window.t === 'function') ? window.t('chatFilesPage.targetConversationCount') : '关联会话';
        const tOpenHint = (typeof window.t === 'function') ? window.t('chatFilesPage.targetOpenHint') : '点击展开归档包';
        const blocks = groups.map(function (g) {
            const rows = g.items.map(function (item) {
                return rowHtml(item.f, item.idx);
            }).join('');
            const heading = chatFilesTargetHeading(g.key, g.items);
            const stats = chatFilesTargetGroupStats(g);
            const titleAttr = heading.title ? ' title="' + escapeHtml(heading.title) + '"' : '';
            const latestText = stats.latest ? new Date(stats.latest * 1000).toLocaleString() : '—';
            const packageCount = (typeof window.t === 'function')
                ? window.t('chatFilesPage.targetPackages', { count: g.items.length })
                : (g.items.length + ' 个归档包');
            const convCount = (typeof window.t === 'function')
                ? window.t('chatFilesPage.targetConversations', { count: stats.conversations })
                : (stats.conversations + ' 个会话');
            const sourceLabels = stats.sources.length
                ? stats.sources.map(chatFilesSourceLabel).join(' / ')
                : chatFilesSourceLabel('upload');
            return `<details class="chat-files-target-group">
                <summary class="chat-files-target-summary"${titleAttr}>
                    <span class="chat-files-target-mark" aria-hidden="true"></span>
                    <span class="chat-files-target-main">
                        <span class="chat-files-target-name">${escapeHtml(heading.text)}</span>
                        <span class="chat-files-target-sub">${escapeHtml(sourceLabels)} · ${escapeHtml(tOpenHint)}</span>
                    </span>
                    <span class="chat-files-target-metrics">
                        <span><strong>${escapeHtml(String(g.items.length))}</strong>${escapeHtml(tPackage)}</span>
                        <span><strong>${escapeHtml(formatChatFileBytes(stats.size))}</strong>${escapeHtml(tSize)}</span>
                        <span><strong>${escapeHtml(convCount)}</strong>${escapeHtml(tConvs)}</span>
                        <span><strong>${escapeHtml(latestText)}</strong>${escapeHtml(tLatest)}</span>
                    </span>
                </summary>
                <div class="chat-files-target-body">
                    <div class="chat-files-target-body-head">${escapeHtml(packageCount)}</div>
                    <table class="chat-files-table">${theadHtml}<tbody>${rows}</tbody></table>
                </div>
            </details>`;
        }).join('');
        innerHtml = `<div class="chat-files-targeted">${blocks}</div>`;
    } else {
        const groups = chatFilesBuildGroups(chatFilesDisplayed, groupMode);
        const blocks = groups.map(function (g) {
            const rows = g.items.map(function (item) {
    ...
    wrap.classList.toggle('chat-files-table-wrap--grouped', groupMode !== 'none' && groupMode !== 'folder' && groupMode !== 'target');
    wrap.classList.toggle('chat-files-table-wrap--tree', groupMode === 'folder');
    wrap.classList.toggle('chat-files-table-wrap--target', groupMode === 'target');
    renderChatFilesPagination();
}
...
async function deleteChatFolderFromBrowse(folderName) {
    ...
    if (!confirm(q)) return;
    try {
        const res = await apiFetch(TRACE_ARCHIVES_API, {
            method: 'DELETE',
...
async function copyChatFolderPathFromBrowse(folderName) {
    ...
    if (relativePath) {
        try {
        const res = await apiFetch(TRACE_ARCHIVES_API + '/path?kind=directory&path=' + encodeURIComponent(relativePath));
            if (!res.ok) {
                const raw = await res.text();
                throw new Error(raw || String(res.status));
            }
...
function chatFilesClipboardFolderPath(segs) {
    ...
    const root = parts.shift();
    if (root === CHAT_FILES_TREE_UPLOAD_ROOT) {
        return ['trace_archives'].concat(parts).join('/');
    }
    if (root === CHAT_FILES_TREE_REDUCTION_ROOT) {
...
async function downloadChatFile(relativePath, filename) {
    try {
        const url = TRACE_ARCHIVES_API + '/download?path=' + encodeURIComponent(relativePath);
        const res = await apiFetch(url);
        if (!res.ok) {
...
async function openChatFilesEdit(relativePath) {
    ...
    try {
        const res = await apiFetch(TRACE_ARCHIVES_API + '/content?path=' + encodeURIComponent(relativePath));
        if (!res.ok) {
            let errText = '';
            try {
...
async function saveChatFilesEdit() {
    ...
    try {
        const res = await apiFetch(TRACE_ARCHIVES_API + '/content', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
function openChatFilesRename(relativePath, currentName) {
    ...
    const pathText = relativePath ? ('trace_archives/' + String(relativePath).replace(/\\/g, '/')) : 'trace_archives';
...
async function submitChatFilesRename() {
    ...
    try {
        const res = await apiFetch(TRACE_ARCHIVES_API + '/rename', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
async function submitChatFilesMkdir() {
    ...
    try {
        const res = await apiFetch(TRACE_ARCHIVES_API + '/mkdir', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function chatFilesUploadFile(file, target) {
    ...
    try {
        ...
        const res = doXhr
            ? await apiUploadWithProgress(TRACE_ARCHIVES_API, form, {
                onProgress: function (p) {
                    chatFilesSetUploadProgressUI(true, p.percent, file.name);
                }
            })
            : await apiFetch(TRACE_ARCHIVES_API, { method: 'POST', body: form });
        if (!res.ok) {
            throw new Error(await res.text());
        }
