const AUTH_STORAGE_KEY = 'hosttrace-auth';
let authToken = null;
...
function openSecurityToolsDownloadModal() {
    if (typeof openAppModal === 'function') {
        openAppModal('security-tools-download-modal');
    } else {
        const modal = document.getElementById('security-tools-download-modal');
        if (modal) modal.style.display = 'flex';
    }
    loadPublicSecurityToolPackages();
}

function closeSecurityToolsDownloadModal() {
    if (typeof closeAppModal === 'function') {
        closeAppModal('security-tools-download-modal');
        return;
    }
    const modal = document.getElementById('security-tools-download-modal');
...
async function loadPublicSecurityToolPackages() {
    const list = document.getElementById('security-tools-public-list');
    if (!list) return;
    list.innerHTML = '<div class="security-tools-empty">' + escapeHtml(authT('securityToolsModal.loading', '正在读取已发布工具包…')) + '</div>';
    try {
        ...
        if (packages.length === 0) {
            list.innerHTML = '<div class="security-tools-empty">' + escapeHtml(authT('securityToolsModal.empty', '暂无已发布工具包，请登录后在系统设置中上传。')) + '</div>';
            return;
        }
        list.innerHTML = packages.map((pkg) => {
            const id = encodeURIComponent(pkg.id || '');
            const href = id ? ('/api/public/security-tools/' + id + '/download') : '/api/public/security-tools/download';
            const size = formatSecurityToolPackageSize(pkg.size || 0);
            return `
                <a class="security-download-card security-download-card--primary" href="${href}" download>
                    <span class="security-download-logo security-download-logo--hosttrace" aria-hidden="true">H</span>
                    <span class="security-download-content">
                        <strong>${escapeHtml(pkg.name || pkg.fileName || authT('securityToolsModal.unnamedPackage', '未命名工具包'))}</strong>
                        <small>${escapeHtml(pkg.description || authT('securityToolsModal.packageDescFallback', '管理员发布的安全工具包，可在未登录状态下载。'))}${size ? ' · ' + escapeHtml(size) : ''}</small>
                    </span>
                    <span class="security-download-action">${escapeHtml(authT('securityToolsModal.downloadZip', '下载 ZIP'))}</span>
                </a>
            `;
        }).join('');
    } catch (err) {
        list.innerHTML = '<div class="security-tools-empty">' + escapeHtml(authT('securityToolsModal.loadFailed', '工具包加载失败')) + ': ' + escapeHtml(err && err.message ? err.message : String(err)) + '</div>';
    }
}
...
async function loadRobotAccountBindings() {
    ...
    const platformLabels = { wecom: '企业微信', dingtalk: '钉钉', lark: '飞书', telegram: 'Telegram', slack: 'Slack', discord: 'Discord', qq: 'QQ' };
