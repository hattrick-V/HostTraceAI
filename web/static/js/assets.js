const ASSET_PAGE_SIZE_KEY = 'hosttrace.asset_page_size';
const ASSET_SAVED_VIEWS_KEY = 'hosttrace.asset_saved_views';
...
const ASSET_IMPORT_COLUMNS = ['target', 'tags', 'host', 'ip', 'domain', 'port', 'protocol', 'title', 'server', 'country', 'province', 'city', 'responsible_person', 'department', 'business_system', 'environment', 'criticality', 'status'];
...
function assetImportRecord(values, rowNumber) {
    ...
    const asset = {
        ...
        project_id: '', source: 'manual-import'
    };
    ...
    return { rowNumber, values, asset, error: '' };
}
...
function renderAssetImportPreview() {
    ...
    if (body) body.innerHTML = rows.slice(0, 100).map(row => {
        ...
        return `<tr class="${row.error ? 'has-error' : ''}"><td>${row.rowNumber}</td><td title="${escapeHtml(target)}">${escapeHtml(target)}</td><td>${escapeHtml(asset.status || row.values.status || '-')}</td><td>${result}</td></tr>`;
    }).join('');
...
const ASSET_FILTER_FIELD_IDS = [
    'asset-search', 'asset-status-filter', 'asset-risk-filter', 'asset-vuln-min-filter',
...
function buildAssetFilterParams() {
    ...
    const map = {
        'asset-search': 'q', 'asset-status-filter': 'status',
...
function ensureAssetOwnershipColumn() {
    ...
    table.querySelectorAll('#asset-table-body td[colspan="8"]').forEach(cell => cell.setAttribute('colspan', '9'));
}
...
function assetExportRows(assets) {
    return assets.map(asset => ({
        ...
        protocol: asset.protocol || '', title: asset.title || '', server: asset.server || '',
...
async function sendAssetsToChat(assets, template) {
    ...
    const response = await apiFetch('/api/conversations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: assetT('assets.scanConversationTitle', `资产扫描：${targets}`, { targets }) }) });
    if (!response.ok) throw new Error(await response.text());
...
async function createAssetScanTasks(assets, template) {
    ...
    const response = await apiFetch('/api/batch-tasks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: assetT('assets.scanQueueTitle', '资产批量扫描'), tasks, executeNow, concurrency: 1, agentMode: 'eino_single', scheduleMode: 'manual' }) });
    if (!response.ok) throw new Error(await response.text());
    const result = await response.json();
...
function importResultToAsset(row, fields) {
    const value = name => {
    ...
    return {
        ...
        source: (window.importState?.currentPayload?.provider || 'imported'), status: 'active'
    };
}
...
async function importFofaAssetsByIndexes(indexes) {
    ...
    const converted = indexes.map(index => importResultToAsset(current.results[index], current.fields));
    ...
    const source = current.provider || 'imported';
