...
var DASHBOARD_SEVERITY_STATUS_FILTER_STORAGE_KEY = 'hosttrace.dashboard.severityStatusFilter';
...
async function refreshDashboard() {
    ...
    try {
        ...
        const [
            tasksRes, traceRes, vulnRes, batchRes, monitorRes, knowledgeRes, skillsRes,
            ...
            severityFilteredStatsRes, tokenUsageRes
        ] = await Promise.all([
            fetchJson('/api/agent-loop/tasks'),
            fetchJson('/api/trace/dashboard-summary?limit=10'),
        ...
        // 运行中对话：仅统计 Agent 循环任务；批量队列见右侧「溯源任务队列」
        ...
        let openLowCount = 0;
        const traceHasData = traceRes && typeof traceRes.findingCount === 'number' && traceRes.findingCount > 0;
        const threatStats = traceHasData
            ? { total: traceRes.findingCount || 0, by_severity: traceRes.bySeverity || {}, by_status: traceRes.byStatus || {} }
            : vulnRes;
        const recentThreatsRes = traceHasData
            ? { vulnerabilities: (traceRes.recentFindings || []).map(function (f) {
                return {
                    id: f.id,
                    title: f.title,
                    severity: f.severity,
                    status: f.status,
                    target: f.address || f.hostName || '',
                    created_at: f.createdAt,
                    _trace: true
                };
            }) }
            : recentVulnsRes;
        if (threatStats && typeof threatStats.total === 'number') {
            if (vulnTotalEl) vulnTotalEl.textContent = String(threatStats.total);
            ...
            const displayBySeverity = traceHasData ? bySeverity : (severityDisplayRes && severityDisplayRes.by_severity || {});
            const displayTotal = traceHasData ? total : (severityDisplayRes && typeof severityDisplayRes.total === 'number' ? severityDisplayRes.total : total);
            ...
            renderVulnStatusPanel(threatStats.by_status || {}, total);
        ...
        // 最近漏洞列表
        renderRecentVulns(recentThreatsRes);
...
function statusShortLabel(s) {
    const k = statusKey(s);
    if (k === 'fixed') return dt('dashboard.statusFixed', null, '已处置');
...
function renderVulnStatusPanel(byStatus, total) {
    ...
    // 处置率只按需要处置的有效漏洞计算；误报/已忽略属于中性闭环，不拉低处置率。
