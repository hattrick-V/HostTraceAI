...
function refreshRobotManager() {
    ['wecom', 'dingtalk', 'lark', 'telegram', 'slack', 'discord', 'qq'].forEach((type) => {
...
function switchSettingsSection(section) {
    ...
    if (section === 'security-tools' && typeof loadSecurityToolsPanel === 'function') {
        setTimeout(() => loadSecurityToolsPanel(false), 0);
    }
}

async function loadSecurityToolsPanel(force) {
    const list = document.getElementById('security-tools-list');
    const countEl = document.getElementById('security-tools-count');
    const packageEl = document.getElementById('security-tools-package-name');
    if (!list) return;
    if (!force && list.dataset.loaded === '1') return;

    list.innerHTML = '<div class="security-tools-empty">' + escapeHtml(settingsT('settingsSecurityTools.loading', '正在读取工具清单…')) + '</div>';
    try {
        const res = await fetch('/api/public/security-tools', { cache: 'no-cache' });
        if (!res.ok) {
            throw new Error(await res.text());
        }
        const data = await res.json();
        const packages = Array.isArray(data.packages) ? data.packages : (Array.isArray(data.tools) ? data.tools : []);
        if (countEl) countEl.textContent = String(data.count || packages.length || 0);
        if (packageEl) packageEl.textContent = packages.length > 0 ? (packages[0].fileName || packages[0].name || '--') : '--';
        if (packages.length === 0) {
            list.innerHTML = '<div class="security-tools-empty">' + escapeHtml(settingsT('settingsSecurityTools.empty', '暂无可发布的安全工具')) + '</div>';
        } else {
            list.innerHTML = packages.map((pkg) => {
                const desc = pkg.description || settingsT('settingsSecurityTools.packageDescFallback', '管理员发布的安全工具包。');
                const size = formatSecurityToolPackageSize(pkg.size || 0);
                const id = encodeURIComponent(pkg.id || '');
                return `
                    <article class="security-tool-card">
                        <code>${escapeHtml(size || 'ZIP')}</code>
                        <h5>${escapeHtml(pkg.name || pkg.fileName || '')}</h5>
                        <p>${escapeHtml(desc)}</p>
                        <div class="security-tool-card-actions">
                            <a class="btn-secondary btn-sm" href="/api/public/security-tools/${id}/download" download>${escapeHtml(settingsT('settingsSecurityTools.download', '下载'))}</a>
                            <button type="button" class="btn-secondary danger btn-sm" onclick="deleteSecurityToolPackage('${settingsEscapeAttr(pkg.id || '')}')">${escapeHtml(settingsT('common.delete', '删除'))}</button>
                        </div>
                    </article>
                `;
            }).join('');
        ...
        list.innerHTML = '<div class="security-tools-empty">' + escapeHtml(settingsT('settingsSecurityTools.loadFailed', '工具清单加载失败')) + ': ' + escapeHtml(err && err.message ? err.message : String(err)) + '</div>';
    }
}

window.loadSecurityToolsPanel = loadSecurityToolsPanel;

async function uploadSecurityToolPackage(file) {
    if (!file) return;
    if (!String(file.name || '').toLowerCase().endsWith('.zip')) {
        alert(settingsT('settingsSecurityTools.onlyZip', '仅支持上传 .zip 工具包'));
        return;
    }
    const nameInput = document.getElementById('security-tools-upload-name');
    const descInput = document.getElementById('security-tools-upload-desc');
    const body = new FormData();
    body.append('file', file, file.name);
    body.append('name', nameInput ? nameInput.value.trim() : '');
    body.append('description', descInput ? descInput.value.trim() : '');
    try {
        const res = await apiFetch('/api/security-tools', { method: 'POST', body });
        if (!res.ok) throw new Error(await res.text());
        if (nameInput) nameInput.value = '';
        if (descInput) descInput.value = '';
        const input = document.getElementById('security-tools-upload-input');
        if (input) input.value = '';
        const list = document.getElementById('security-tools-list');
        if (list) list.dataset.loaded = '0';
        await loadSecurityToolsPanel(true);
    } catch (err) {
        alert((err && err.message) ? err.message : String(err));
    }
}

async function deleteSecurityToolPackage(id) {
    if (!id) return;
    if (!confirm(settingsT('settingsSecurityTools.deleteConfirm', '确定删除这个已发布工具包吗？'))) return;
    try {
        const res = await apiFetch('/api/security-tools/' + encodeURIComponent(id), { method: 'DELETE' });
        if (!res.ok) throw new Error(await res.text());
        const list = document.getElementById('security-tools-list');
        if (list) list.dataset.loaded = '0';
        await loadSecurityToolsPanel(true);
...
function formatSecurityToolPackageSize(size) {
    size = Number(size || 0);
...
window.uploadSecurityToolPackage = uploadSecurityToolPackage;
window.deleteSecurityToolPackage = deleteSecurityToolPackage;
