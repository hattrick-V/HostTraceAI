...
# HostTraceAI one-click deploy and start script
...
print_banner() {
    ...
    echo ""
    echo "=========================================="
    echo "  HostTraceAI Deploy & Start Script"
...
BINARY_NAME="hosttrace-ai"

# Check Python environment.
#
# Python is OPTIONAL: the platform itself (Go server + web console + built-in
# forensics tools) runs without it. Python packages in requirements.txt are only
# needed by the optional MCP helper scripts under mcp-servers/.
# Returns 0 when python3 is usable, 1 otherwise -- callers decide what to do.
check_python() {
    if ! command -v python3 >/dev/null 2>&1; then
        warning "python3 not found; skipping optional Python dependencies"
        info "The platform runs fine without Python. It is only needed by the"
        info "optional helper scripts under mcp-servers/."
        echo ""
        return 1
    fi
    ...
    if [ "$PYTHON_MAJOR" -lt 3 ] || ([ "$PYTHON_MAJOR" -eq 3 ] && [ "$PYTHON_MINOR" -lt 10 ]); then
        warning "Python version too old: $PYTHON_VERSION (optional deps want 3.10+)"
        info "Skipping optional Python dependencies."
        echo ""
        return 1
    fi
...
# Set up Python virtual environment (optional; skipped when python3 is absent).
setup_python_env() {
    if [ ! -d "$VENV_DIR" ]; then
        info "Creating Python virtual environment..."
        python3 -m venv "$VENV_DIR" || {
            warning "Failed to create virtual environment; skipping optional Python deps"
            return 0
        }
    ...
    source "$VENV_DIR/bin/activate" || {
        warning "Failed to activate virtual environment; skipping optional Python deps"
        return 0
    }
...
main() {
    ...
    if check_python; then
        ...
        info "Skipping Python setup (optional)"
    fi
    echo ""
    ...
    if [ "$USE_HTTPS" -eq 1 ]; then
        info "Starting HostTraceAI server (HTTPS + HTTP/2, self-signed cert)..."
        note "For plain HTTP, use: $0 --http"
    else
        info "Starting HostTraceAI server (HTTP)..."
    fi
    echo "=========================================="
    echo ""
